...
export function LikersDialog({ postId, open, onOpenChange }: Props) {
  const { t } = useTranslation();
  const currentUser = useAuthStore((state) => state.account);
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } = useLikers(postId, open);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[400px] h-[80vh] sm:h-[600px] flex flex-col p-0 gap-0 overflow-hidden">
        <DialogHeader className="p-4 border-b shrink-0">
          <DialogTitle className="text-lg">{t('post.likers.title')}</DialogTitle>
        </DialogHeader>
