    ...
    return (
      <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500 text-center">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-bold tracking-tight">{t('auth.forgotPassword.checkEmailTitle')}</h1>
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground font-light text-lg">{t('auth.forgotPassword.subtitle')}</p>
      </div>

      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5">
        <div className="space-y-2">
          <Label htmlFor="email" className={errors.email ? "text-destructive" : ""}>{t('auth.forgotPassword.emailLabel')}</Label>
          <Input
            id="email"
            type="email"
        ...
        <Button type="submit" className="w-full h-12 text-base font-semibold transition-transform active:scale-95" disabled={mutation.isPending}>
          {mutation.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : t('auth.forgotPassword.submit')}
        </Button>
      </form>
