...
function Avatar({ account, size }: { account: PublicAccountResponse; size: number }) {
  ...
  return (
    <View
      ...
      style={{ width: size, height: size, borderRadius: size / 2 }}
    >
      <Text className="font-bold text-neutral-700 dark:text-neutral-200" style={{ fontSize: size * 0.35 }}>
  ...
  return (
    <View>
      ...
      <View className="px-4 pb-3">
          ...
          <View className="flex-row items-center gap-2 pt-3">
            {isOwn ? (
              <>
                ...
                >
                  <Text className="font-bold text-neutral-900 dark:text-neutral-50">Edit profile</Text>
                </Pressable>
              </>
            ) : (
              <>
        ...
        {profile.bio ? (
          <Text className="mt-3 text-[15px] text-neutral-900 dark:text-neutral-50">{profile.bio}</Text>
        ) : null}
        ...
        <View className="mt-3 flex-row gap-5">
          ...
          >
            <Text className="font-bold text-neutral-900 dark:text-neutral-50">{profile.followingCount}</Text>
            <Text className="text-neutral-500">Following</Text>
          </Pressable>
          <Pressable
            className="flex-row gap-1"
            onPress={() =>
          ...
          >
            <Text className="font-bold text-neutral-900 dark:text-neutral-50">{profile.followerCount}</Text>
            <Text className="text-neutral-500">Followers</Text>
          </Pressable>
        </View>
      </View>
      ...
      <View className="flex-row border-b border-neutral-100 dark:border-neutral-800">
        {(['posts', 'replies', 'likes'] as const).map((tab) => (
          <Pressable key={tab} className="flex-1 items-center py-3" onPress={() => onTabChange(tab)}>
            <Text
              className={
                activeTab === tab
            ...
            {activeTab === tab && <View className="mt-2 h-1 w-12 rounded-full bg-primary" />}
          </Pressable>
        ))}
      </View>
    </View>
  ...
  if (profileQuery.status === 'error' || !profileQuery.data) {
    return (
      <View className="flex-1 items-center justify-center bg-white px-8 dark:bg-neutral-950">
        <Text className="text-center text-neutral-500">User not found.</Text>
      </View>
    );
  }
      ...
      <FlatList
        ...
        ListEmptyComponent={
          activeQuery.status === 'pending' ? (
            ...
            <View className="items-center py-10">
              <Text className="text-neutral-500">Nothing here yet.</Text>
            </View>
          )
        }
        ListFooterComponent={
